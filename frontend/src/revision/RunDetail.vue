...
const detailTab=ref(new URLSearchParams(location.hash.split('?')[1]??'').get('tab')==='analysis'||location.hash.startsWith('#optimizer/')?'analysis':'results')
...
<template>
 ...
 <section v-if="progress" class="task-overview" aria-label="任务结果总览">
  ...
  <details v-if="evidence" class="overview-config"><summary>本次任务配置与版本来源</summary><RunConfiguration :manifest="evidence.run.manifest"/><RunLineage :run-id="id"/></details>
 </section>
 ...
 <div v-if="report" class="tabs" aria-label="任务详情内容"><button :class="['tab',{active:detailTab==='results'}]" @click="detailTab='results'">评测结果</button><button :class="['tab',{active:detailTab==='analysis'}]" @click="detailTab='analysis'">调优分析</button><button :class="['tab',{active:detailTab==='static'}]" @click="detailTab='static'">Skill 静态分析</button></div>
 ...
 <template v-if="evidence&&detailTab==='results'">
  ...
  <div class="report-layout case-report-layout">
   ...
   <div v-if="sample"><div v-if="results.some(r=>r.outcome==='fail')" class="toolbar"><button class="secondary" :disabled="busy" @click="writeback">回写失败用例到草稿</button></div><CaseReport v-if="sample" :key="caseId" :sample="sample" :results="results" :primary-ids="evidence.run.manifest.primary_evaluator_ids" :trace="trace" :trace-error="traceError" :trace-loading="traceLoading" :complete="!!report" hide-analyze/></div>
   <section v-else class="card empty">暂无可展示的样本结果</section>
  </div>
 </template>
</template>
<style scoped>
.task-overview{background:white;border:1px solid #dfe8e5;border-radius:16px;padding:24px;margin-bottom:24px;box-shadow:0 4px 18px #173b2a05}.overview-heading{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}.overview-heading h2{font-size:21px;margin:6px 0 0}.overview-eyebrow{font-size:12px;letter-spacing:1px;color:#718078}.overview-scores{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px}.score-tile{padding:20px;border:1px solid #e4eae8;background:#f8faf9;border-radius:12px}.score-tile.primary{background:linear-gradient(125deg,#e8f7f1,#f5fbf8);border-color:#bbdfd0}.score-tile>span{font-size:14px;color:#50675d}.score-number{font-size:38px;font-weight:650;letter-spacing:-1px;margin:10px 0;color:#183a2d}.score-number small{font-size:14px;font-weight:400;color:#83928a;margin-left:8px;letter-spacing:0}.score-tile p{font-size:12px;color:#65796f;margin:0;line-height:1.8}.score-caption{font-size:12px;color:#7c8882;margin:10px 0 20px}.overview-facts{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:24px;padding:20px 0}.overview-facts>div{display:flex;flex-direction:column;gap:8px;min-width:0}.overview-facts span,.overview-facts small{font-size:12px;color:#758078}.overview-facts b{font-size:15px;overflow-wrap:anywhere}.overview-facts small{overflow-wrap:anywhere}.overview-config{border-top:1px solid #e8ecea;padding-top:16px;font-size:13px}.overview-config summary{color:#008570;cursor:pointer}@media(max-width:800px){.overview-facts{grid-template-columns:1fr 1fr}.task-overview{padding:16px}.overview-scores{grid-template-columns:1fr}}
