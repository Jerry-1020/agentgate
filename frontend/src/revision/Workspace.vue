...
import {readTaskLinks,refreshTaskLinks,type TaskLink} from './task-links'
...
function groupStatus(run:EvaluationRun,records=runs.value){
 const link=linkFor(run.id);if(!link||link.runIds.length===1)return run.status
...
function go(value:string) { const id=value.split('/')[1];location.hash = value.startsWith('tasks/')&&linkFor(id)?.kind==='stability'?'stability/'+id:value }
...
async function loadPage(){
 ...
 return (link?.runIds??[r.id]).some(id=>{const item=records.find(x=>x.id===id);return JSON.stringify([id,item?.manifest.target.display_name,item?.manifest.target.ref.external_version_id,item?.manifest.dataset.dataset_name]).toLowerCase().includes(query.value.toLowerCase())})&&(!taskType.value||(link?.kind==='stability'?'single':link?.kind??'single')===taskType.value)&&(!status.value||groupStatus(r,records)===status.value)&&(!datasetFilter.value||r.manifest.dataset.dataset_id===datasetFilter.value)&&(!evaluatorFilter.value||r.manifest.primary_evaluator_ids.includes(evaluatorFilter.value))
...
async function refresh() {
 ...
 loading.value=true
 try { const [o,r,links] = await Promise.all([api.overview(),api.runs(),refreshTaskLinks()]); overview.value=o; runs.value=r;taskLinks.value=links; online.value=true; error.value='';await loadPage() }
...
async function taskCreated(link:TaskLink){create.value=false;taskLinks.value=readTaskLinks();go((link.kind==='stability'?'stability/':'tasks/')+link.id);await refresh()}
</script>
<template>
 <div class="app revision-app">
  <header class="topbar"><div class="brand">智能体评测中心 <small>评测工作台</small></div><span class="muted">评测对象以任务运行快照为准</span><span class="badge" :class="online?'success':'warn'">{{online===null?'正在连接服务…':online?'评测后端已连接':'服务未连接'}}</span></header>
  <div class="shell"><aside class="sidebar"><template v-for="group in navigation" :key="group.title"><div class="nav-group">{{group.title}}</div><a v-for="item in group.items" :key="item[0]" :href="'#'+item[0]" class="nav-item" :class="{active:page===item[0]||(page==='stability'&&item[0]==='tasks')}">{{item[1]}}</a></template><div class="sidebar-foot">AgentGate 评测工作台<br>数据库结果 · 执行证据追溯</div></aside>
   <main class="content">
