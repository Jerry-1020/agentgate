...
async function launch() {
  ...
  if (selectedDatasetInfo.value?.version == null) return ElMessage.warning('请选择已有发布版本的评测集')
  loading.value = true
  try {
    ...
    <div class="app-content">
      <header class="page-header">
        ...
        <div>
          ...
          <h1>{{ page === 'evaluate' ? 'AgentGate 评估台' : page === 'runs' ? '运行队列' : '评测集管理' }}</h1>
          <p>{{ page === 'evaluate' ? '配置评估对象，运行用例，并用可追溯指标判断是否达到发布门槛。' : page === 'runs' ? '跟踪排队、执行进度与最近结果。' : '维护评测集、不可变版本与可复用测试用例。' }}</p>
        </div>
      </header>
