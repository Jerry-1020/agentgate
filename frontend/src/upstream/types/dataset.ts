...
export type SerializedExpectation = Expectation
  | {id:string;name:string|null;kind:'skill_route';condition:Condition}
  | {id:string;name:string|null;kind:'tool_call';tool:string;mode:'required'|'forbidden'}
  | {id:string;name:string|null;kind:'policy';policy_id:string}
export interface CaseTurn {
  ...
  policy_rules: string[]
  notes: string
  original_expectations?: SerializedExpectation[]
  preserved_skill_routes?: { id: string; name: string | null; kind: 'skill_route'; condition: Condition }[]
}
