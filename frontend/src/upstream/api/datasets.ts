...
import type {
  ...
  DatasetVersion, EvaluationCase, Expectation, SerializedExpectation,
} from '../types/dataset'
...
type ApiCaseTurn = Omit<CaseTurn,
  'expected_skill' | 'expectations' | 'required_tools' | 'forbidden_tools' | 'policy_rules' | 'preserved_skill_routes' | 'original_expectations'
      ...
      return {
        ...
        preserved_skill_routes: turn.expectations.filter(
          (item): item is ApiExpectationBase & {kind:'skill_route';condition:Condition} =>
            item.kind === 'skill_route' && (item !== skill || item.condition.kind !== 'equals' || typeof item.condition.expected !== 'string'),
        ),
...
export function toApiCase(item: EvaluationCase): ApiEvaluationCase {
  return {
    ...item,
    turns: item.turns.map(({expected_skill,required_tools,forbidden_tools,policy_rules,preserved_skill_routes,original_expectations,...turn})=>{
      const original=original_expectations??[],used=new Set<string>()
      function reuse(candidate:SerializedExpectation,predicate:(e:SerializedExpectation)=>boolean):SerializedExpectation {
      ...
      const expectations:SerializedExpectation[]=[
        ...
        ...(expected_skill?[reuse({id:crypto.randomUUID(),kind:'skill_route',name:null,condition:{kind:'equals',expected:expected_skill}},
          e=>e.kind==='skill_route'&&e.condition.kind==='equals'&&typeof e.condition.expected==='string'&&!preserved_skill_routes?.some(p=>p.id===e.id))]:[]),
        ...required_tools.map(tool=>reuse({id:crypto.randomUUID(),kind:'tool_call',name:null,tool,mode:'required'},
          e=>e.kind==='tool_call'&&e.mode==='required'&&e.tool===tool)),
        ...forbidden_tools.map(tool=>reuse({id:crypto.randomUUID(),kind:'tool_call',name:null,tool,mode:'forbidden'},
          e=>e.kind==='tool_call'&&e.mode==='forbidden'&&e.tool===tool)),
        ...policy_rules.map(policy_id=>reuse({id:crypto.randomUUID(),kind:'policy',name:null,policy_id},
          e=>e.kind==='policy'&&e.policy_id===policy_id)),
        ...turn.expectations,
      ]
      const order=new Map(original.map((e,i)=>[e.id,i]))
      expectations.sort((a,b)=>(order.get(a.id)??Number.MAX_SAFE_INTEGER)-(order.get(b.id)??Number.MAX_SAFE_INTEGER))
...
export const datasetApi = {
  ...
  addCase: (datasetId: string, item: EvaluationCase, expectedHash: string) =>
    request<ApiDatasetVersion>(`/api/datasets/${id(datasetId)}/drafts/cases`, {
      method: 'POST', headers: headers, body: JSON.stringify(toApiCase(item)),
    }).then(toEditorVersion),
  ...
  reorderCases: (datasetId: string, caseIds: string[], expectedHash: string) =>
    request<ApiDatasetVersion>(`/api/datasets/${id(datasetId)}/drafts/case-order`, {
      method: 'PUT', headers: {...headers,'If-Match':expectedHash}, body: JSON.stringify({ case_ids: caseIds }),
    }).then(toEditorVersion),
