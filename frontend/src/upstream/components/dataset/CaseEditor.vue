...
const baseline=ref('')
function snapshot(){return JSON.stringify([form.value,inputs.value,outputs.value])}
watch([form,inputs,outputs],()=>emit('dirtyChange',snapshot()!==baseline.value),{deep:true})
...
watch(
  ...
  item => {
    ...
    outputIds.value = item?.turns.map(turn => turn.expectations.find(e=>e.kind==='output'&&!e.path&&e.condition.kind==='equals')?.id??null)??[]
    originalOutputs.value=item?.turns.map((turn,index)=>cloneJson(turn.expectations.find(e=>e.id===outputIds.value[index])??null))??[]
    outputs.value = item?.turns.map((turn,index) => {
    ...
    form.value?.turns.forEach((turn:any,index:number)=>{turn.expectations=turn.expectations.filter((e:any)=>e.id!==outputIds.value[index])})
...
function save() {
  ...
  try {
    saved.turns.forEach((turn:any,index:number)=>{
      if(outputs.value[index]?.trim())turn.expectations.push({
        ...(originalOutputs.value[index]??{id:outputIds.value[index]??crypto.randomUUID(),kind:'output',name:'期望输出',path:null}),
        condition:{kind:'equals',expected:JSON.parse(outputs.value[index])},
      })
    })
  } catch { return ElMessage.error('期望输出必须为有效 JSON；文本请用双引号包围') }
  emit('save', saved)
}
defineExpose({save})
</script>
  ...
  <section class="dataset-column case-editor-panel">
    ...
    <el-form v-else label-position="top" class="case-editor-form" :disabled="!editable">
      ...
      <div class="case-meta-grid">
        ...
        <el-form-item label="分类"><template #label><span title="正例：应正常完成；负例：应正确拒绝或处理异常；边界：临界条件。分类不会直接决定评测通过与否。">分类 ⓘ</span></template>
      ...
      <details class="optional-case-meta"><summary>更多信息 · 难度、标签与备注</summary>
      <div class="case-meta-grid">
      ...
      <details v-if="form.initial_state&&Object.keys(form.initial_state).length" class="state-snapshot"><summary>已保存的前置状态（只读保留）</summary><pre>{{JSON.stringify(form.initial_state,null,2)}}</pre></details>
        ...
        <el-collapse-item v-for="(turn, index) in form.turns" :key="turn.id" :name="turn.id">
          ...
          <div class="turn-form">
            ...
            <el-form-item label="期望输出（JSON）">
              <el-input v-model="outputs[Number(index)]" type="textarea" :rows="4" :data-testid="`turn-output-${index}`" placeholder='例如 {"status":"pending_review"} 或 "已转人工复核"' />
              <small>与本轮完整实际输出精确比较，由“最终输出”规则评估器检查；不传给智能体。留空则不添加此项检查。</small>
            </el-form-item>
            <details class="other-expectations" :open="!editable"><summary>{{editable?'添加其他期望':'其他期望'}} · 已配置 {{(turn.expected_skill?1:0)+(turn.required_tools?.length??0)+(turn.forbidden_tools?.length??0)+(turn.policy_rules?.length??0)+(turn.expectations?.length??0)}} 项</summary><p class="field-hint">路由由 Skill 规则检查，工具约束由工具规则检查；本页维护样本期望，任务中选择要执行的评估器。</p>
