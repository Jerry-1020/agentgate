...
test('dataset launches the viewed published version through the shared task form',async({page})=>{
 ...
 await expect(dialog.getByLabel('任务评测集',{exact:true})).toHaveValue('loan-risk-policy')
 await expect(dialog.getByLabel('任务评测集版本',{exact:true})).toHaveValue('1')
 await expect(dialog.getByLabel('任务评测集版本',{exact:true})).toBeDisabled()
 await expect(dialog.getByLabel('任务评测集',{exact:true})).toBeDisabled()
