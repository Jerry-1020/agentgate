...
test('LLM form selects configured models and hides internal fields',async({page})=>{
 await page.route('**/api/configured-models',r=>r.fulfill({json:[{provider_id:'test-service',model_id:'test-judge',credential_ref:'env:TEST_KEY'}]}))
...
test('configuration reports an empty connected-model catalog honestly',async({page})=>{
 await page.route('**/api/configured-models',r=>r.fulfill({json:[]}))
