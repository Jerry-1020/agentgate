...
test('delete draft, archive and restore published dataset, confirm run scope',async({page,request})=>{
 ...
 try{
  ...
  await page.reload();await page.getByLabel('搜索评测集',{exact:true}).fill(cid)
  ...
  await expect(row).toHaveCount(0)
  await page.getByLabel('评测集状态',{exact:true}).selectOption('archived');await expect(row).toBeVisible()
  await row.getByRole('button',{name:'恢复',exact:true}).click();await expect(row).toHaveCount(0)
  await page.getByLabel('评测集状态',{exact:true}).selectOption('all')
  ...
  await dialog.getByLabel('任务评测集',{exact:true}).selectOption(cid)
